...
import { Search, MapPin, Star, Filter, X, Phone, Mail, Globe, CheckCircle, Send, User, ChevronRight, Eye, Copy, Check, Clock, Edit, Sparkles, XCircle } from 'lucide-react';
...
const ExplorePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (location.state && location.state.aiResults) {
        ...
        // Clear history state to prevent reload showing same results (optional)
              ...
              <div className="space-y-2">
                ...
                <label className="flex items-center gap-2 cursor-pointer hover:bg-slate-50 p-1 rounded">
                  <input 
                    ...
                    disabled={!!aiResults}
                  />
                  <span className={`text-sm ${aiResults ? 'text-slate-400' : 'text-slate-700'}`}>All Industries</span>
                </label>
                {INDUSTRIES.map(ind => (
                  <label key={ind.id} className="flex items-center gap-2 cursor-pointer hover:bg-slate-50 p-1 rounded">
                    <input 
                      ...
                      disabled={!!aiResults}
                    />
                    <span className={`text-sm ${aiResults ? 'text-slate-400' : 'text-slate-700'}`}>{ind.name}</span>
                  </label>
                ))}
              </div>
            </div>
              ...
              {filteredBusinesses.map((biz) => (
                <div key={biz.id} className={`bg-white rounded-xl shadow-sm border overflow-hidden hover:shadow-md transition-shadow group flex flex-col h-full relative ${biz.score && biz.score > 0.8 ? 'border-voxa-gold/40 shadow-voxa-gold/10' : 'border-slate-100'}`}>
                  ...
                  <div className="p-6 flex-grow flex flex-col">
                    ...
                    <div className="text-xs text-slate-400 mb-2 font-medium">{biz.entityType || 'Business'}</div>
